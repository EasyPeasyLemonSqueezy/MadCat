    ...
    public static class TopologicalSort
    {
            ...
            Func<T, TKey> getKey)
        {
            ...
            return item => {
                ...
                return dependencies?.Where(key => map.ContainsKey(key)).Select(key => map[key]);
            };
        }

        public static IList<T> Sort<T, TKey>(
            IEnumerable<T> source,
            Func<T, IEnumerable<TKey>> getDependencies,
            Func<T, TKey> getKey,
        ...
        public static IList<ICollection<T>> Group<T, TKey>(
            IEnumerable<T> source,
            Func<T, IEnumerable<TKey>> getDependencies,
            Func<T, TKey> getKey,
            bool ignoreCycles = true)
        {
            var source2 = (source as ICollection<T>) ?? source.ToArray();
        ...
        public static IList<ICollection<T>> Group<T>(
            IEnumerable<T> source,
            Func<T, IEnumerable<T>> getDependencies,
            IEqualityComparer<T> comparer = null,
            bool ignoreCycles = true)
        ...
        public static int Visit<T>(
            T item,
            Func<T, IEnumerable<T>> getDependencies,
            List<ICollection<T>> sorted,
